...
const enqueue = (docs) => {
  console.log('>>>>>>>>>>>>> enqueue!', docs);

  amqp.connect('amqp://B8Xdg5GXyA0uQ-NE:nTUuQ3YEZnQ5IsuA@localhost:5672',
  (err, conn) => {
    conn.createChannel((err, ch) => {
      const qName = 'abacus-queue';

      // ch.publish(exchangeName, bindingKey, new Buffer(JSON.stringify(docs)));
      // console.log('==========>', msg);
      ch.assertQueue(qName, { durable: true });
      const resp = ch.sendToQueue(qName, new Buffer(JSON.stringify(docs[0])),
         { persistent: true });
