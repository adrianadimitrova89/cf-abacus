...
const lastAccum = function *(idoc, okey, otime, ocache, odb) {
  ...
  const key = dbclient.kturi(okey, t);
  ...
  if (!odocs || !odocs.rows.length) {
    ...
    opt.query = {
      _id: key,
      $or: [{ _state: 'UNLOCKED' }, { _state: null }]
    };
    opt.update = {
      _state: 'LOCKED',
      _initial: true
  ...
  } else if (odocs.rows[0].doc._state === 'LOCKED') {
    // retry
    console.log('Not Implemented (locked)');
  } else {
    // lock the doc
    // update opt
    opt.query = {
      _id: odocs.rows[0].doc.id,
      _state: 'UNLOCKED'
    }
    opt.update = {
      _state: 'LOCKED'
    ...
    opt.upsert = false;
    opt.new = true;
    console.log('Not Implemented (unlocked)');
  }

  const doc = yield odb.findAndModify(opt);
  console.log('LASTDOC: %j', doc);
  // if(!odocs || !odocs.rows.length) {
  //   debug('No existing accumulated output doc since %s', eid);
  //   return undefined;
  // }
  ...
  debug('Retrieved accumulated output doc id %s, rev %s, %o',
    doc.id, doc.rev, doc);
  return dbclient.undbify(extend({}, doc.val, {
    rev: doc._rev
  }));
};
...
const groupReduce = (itype,
  ...
  return yieldable(batch(batch.groupBy(function *(calls) {
    ...
    try {
      ...
      const getLastAccum = function *(kts) {
          if(!odb)
            return {};
          return yield lastAccum(
            calls[0][0].idoc, kts[0], kts[1], ocache, odb);
      ...
      const kts = zip(calls[0][0].okeys, calls[0][0].otimes);
      let accums = [];

      for(let i = 0; i < kts.length; i++) {
        accums[i] = yield getLastAccum(kts[i]);
      }

      console.log('========ACCUMS: %j', accums);
