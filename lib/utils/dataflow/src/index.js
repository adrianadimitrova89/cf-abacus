...
const postOutput = function *(odoc, skey, stime,
  shost, spartition, spost, authentication, ddup, ctx) {
...
const postOutputs = function *(odocs, skeys, stimes,
  shost, spartition, sposts, authentication, ddup, ctx) {
  ...
  const responses = yield tmap(odocs, function *(odoc, i, l) {
    return yield postOutput(odoc, skeys[i], stimes[i],
      shost, spartition, sposts[i], authentication, ddup, ctx);
  });
...
const mapper = (mapfn, opt) => {
  ...
  const play = function *(idoc, auth, ctx) {
    ...
    try {
      ...
      if(ddup)
        yield tracing.runSpan(ctx, 'detect-duplicate', function *() {
          yield detectDup(oid, ddup, odb);
        });

      // Log the input doc
      if(idb)
        yield tracing.runSpan(ctx, 'store-input-document', function *() {
      ...
      const odocs = yield ymapfn(playedInputDoc, auth, ctx);
      ...
      if(opt.sink.host && opt.sink.posts && !error)
        error = yield tracing.runSpan(ctx, 'send-to-sink', function *(sendCtx) {
          return yield postOutputs(podocs, skeys, stimes,
            opt.sink.host, opt.sink.apps, opt.sink.posts,
            opt.sink.authentication, ddup, sendCtx);
        });

      // Log the output docs when there is no error encountered
      if(odb && !error)
        yield tracing.runSpan(ctx, 'store-output-document', function *() {
      ...
      if(edb && error) {
        ...
        if(!doc) {
          ...
          yield tracing.runSpan(ctx, 'store-error-document', function *() {
  ...
  routes.post(opt.input.post, throttle(function *(req) {
    ...
    const ctx = req.context || rootContext;

    const played = yield tracing.runSpan(ctx, 'dataflow-map', function *(
      mapCtx) {

      return yield play(idoc, getAuthorization(req), mapCtx);
    });
      ...
      const presults = yield tmap(pgdocs,
        function *(podocs, i, l) {
          ...
          return yield postOutputs(podocs,
            ...
            shost, spartition, sposts, calls[i][0].authentication, ddup, ctx);
        });
...
const reducer = (reducefn, opt) => {
  ...
  routes.post(opt.input.post, throttle(function *(req) {
    ...
    const played = yield tracing.runSpan(ctx, 'dataflow-reduce', function *(
          reduceCtx) {
    
        return yield play(idoc, getAuthorization(req), reduceCtx);
    });
