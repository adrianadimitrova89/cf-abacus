...
describe('abacus-schema', () => {
  ...
  it('validates a document that contains functions with valdators', () => {
    const validate = schema.validator(schema(objectType('doc', {
      a: functionString('x')
    })), undefined, {
      'x': () => {
        throw new Error('x functions not allowed');
      }
    });
        ...
        expect(err.message).to.deep.equal([
          {
            type: 'string',
            field: 'data.a',
            message: 'must be x-function format',
            value: '(x) => x'
          },
          {
            message: 'x functions not allowed',
            source: '(x) => x'
          }
        ]);
      }
