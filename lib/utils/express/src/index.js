...
const threshold = process.env.THRESHOLD || 90;

// Checks the app memory,
// If it is over the configured threshold it will reject the request
const memoryUsage = () => {
  console.log('Starting memory usage middleware.....');
  return (req, res, next) => {
    const mem = process.memoryUsage();
    const used = mem.heapUsed / mem.heapTotal * 100;

    if(used >= threshold) {
      console.log('----------> ACHTUNG! ACHTUNG! PANIC! OVERLOAD!!!')
      res.status(429).send('Server overload');
      return;
    } else {
      console.log('-----------> DAS IST NORMAL OPERATIONE: ', used);
    }
