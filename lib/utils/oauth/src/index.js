...
const extractClientId = (auth) => {
  let clientId;
  try {
    clientId = decode(auth.replace(/^bearer /i, '')).payload.client_id;
  }
  catch(exception) {
    edebug('Exception when extracting Client id, %o', exception);
    debug('Exception when extracting Client id, %o', exception);
...
module.exports.extractClientId = extractClientId;
