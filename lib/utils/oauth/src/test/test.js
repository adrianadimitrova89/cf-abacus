...
describe('abacus-oauth', () => {
  ...
  context('when getting client_id from token', () => {
    it('should fail with empty token', () => {
      let clientId;
      try {
        clientId = oauth.extractClientId('');
      }
      catch(e) {
    ...
    it('should fail with invalid token', () => {
      let clientId;
      try {
        clientId = oauth.extractClientId('This is invalid token');
      }
      catch(e) {
        expect(clientId).to.be.undefined;
        expect(e.statusCode).to.equal(401);
    ...
    it('should succeed with valid token', () => {
      const clientId = oauth.extractClientId(encodedToken);
      expect(clientId).to.be.equal(testClientId);
    });
  });
});
