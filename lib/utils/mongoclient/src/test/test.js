...
describe('abacus-mongoclient', () => {
  ...
  context.only('findAndModify tests', () => {
    it('upsert option', (done) => {
      const berr = new Error('Can\'t load balance DB partition 3');
      const perr = new Error('Can\'t en DB partition 2-201411');
      ...
      const findAndModify = (done) => {
        ...
        console.log('findAndModify before put');
        ...
        console.log('findAndModify after put, before findAndModify');

        db.findAndModify({
          id: helloKey,
          _state: 'LOCKED'
        },
        {_state: 'UNLOCKED'}, (err, doc) => cb(expect(err).to.equal(null)));

      };
      clean(() => findAndModify(done));
    });
  });
