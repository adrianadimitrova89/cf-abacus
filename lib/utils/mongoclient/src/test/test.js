...
describe('abacus-mongoclient', () => {
  ...
  const missingKey = dbops.kturi('Missing', moment.utc(
    '2014-11-06 19:06:54').valueOf());
    // '2014-09-06 19:06:54').valueOf()); // it works with this key
  ...
  it('distributes bulk operations over 4 db partitions', (done) => {
    ...
    const getListWithMissingKey = (done) => {
      debug('bulk: starting getlist when key is missing ...');
      // Expect to get the requested docs from partitions 0 and 1
      db.allDocs({
        include_docs: true,
        keys: [
        ...
        expect(docs.rows[1].error).to.equal('not_found');
    ...
    clean(() => putlist(() => verifyPutlist(() => puterr(() => get(() =>
      getlist(() => getListWithMissingKey(() => geterr(() => getrange(() =>
      getNoDocs(() => getDocsAscending(() => getDocsDescending(() =>
        getlimit(() => getrangeerr(done))))))))))))));
  });
