...
const mongoclient = (partitionFn, uriFn, connectionFn, errorDBFn) => {
  ...
  return {
    ...
    // Find a single document and update it if rev is the same
    findAndModify: (prevDoc, updatedDoc, cb) => {
      console.log('mongoclient::findAndModify');
      edebug('mongoclient::findAndModify');
      dbops.singleOp((db, prevDoc, cb) => {
        const collection = db.collection(getCollectionName(db));
        collection.findAndModify(
          ...
          { $set: updatedDoc },
          {
            ...
            console.log('INNER-find-and-modify %j %j', err, doc);
            cb(err, doc);
          });
      }, 'read-write', dbops.dbify(prevDoc), dbopt.partition, pool,
        (err, val) => {
          // TODO: perf report
          console.log('OUTER-find-modify %j\n %j', err, val);
          cb(err, val);
        }
      );
    ...
    // batch version of findAndModify
    batch_findAndModify: () => {
