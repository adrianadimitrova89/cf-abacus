...
describe('abacus-metering-schemas', () => {
  describe('validate schema for a metering plan', () => {
      ...
      if (expectedError) {
        ...
        expect(error).to.deep.equal({
          ...
          message: [{
            ...
            message: 'must be aggregate-function format',
            type: 'string',
    ...
    it('reports an invalid plan containing invalid functions (ESLint)', () => {
      const aggregateFunction = `let ${unusedVar} = 2`;
      validateMeteringPlan(aggregateFunction, true);
    });

    it('reports a valid plan containing valid functions (ESLint)', () => {
    ...
    it('reports an invalid plan containing invalid functions (eval)', () => {
      const def = {
        plan_id: 'test',
        measures: [{
          name: 'heavy_api_calls',
        ...
        metrics: [{
          ...
          unit: 'CALL',
          type: 'discrete',
          accumulate: '42', // not a function
          aggregate: '() => { var i = 0; while (i >= 0) i++; }' // infinite loop
        }]
      };
      ...
      expect(error).to.deep.equal({
        statusCode: 400,
        message: [{
          field: 'data.metrics.0.accumulate',
          message: 'must be accumulate-function format',
          type: 'string',
          value: '42'
        },
        {
          ...
          message: 'must be aggregate-function format',
          type: 'string',
          value: '() => { var i = 0; while (i >= 0) i++; }'
        },
        {
          message: 'Function.prototype.apply was called on 42, ' +
            'which is a number and not a function',
          source: '42'
        },
        {
          message: 'Script execution timed out.',
          source: '() => { var i = 0; while (i >= 0) i++; }'
        }]
      });
    });
