...
const create = (opts) => {
  ...
  const createRetriever = opts.createRetriever;
  ...
  const processEvent = function *(event) {
    ...
    yield adjustTimestamp(usage, event.metadata.guid);
    yield sendUsage(usage, event);
    yield progress.save({
  ...
  const pollEvents = (callback) => {
    ...
    const retriever = createRetriever(progress.get().guid);
    retriever.forEachEvent(functioncb(function *(event) {
    ...
    retriever.whenSucceeded(() => {
      debug('Polling events was successful');
      callback();
    });
    retriever.whenFailed((err) => {
      functioncb(function *() {
        ...
        // assure we fail with orignal error
        throw err;
      })(callback);
    });
