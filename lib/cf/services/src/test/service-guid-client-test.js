...
// const verifyArgs = (actualUrl, actualToken, actualPerf, actualStatistics) =>
// {
//   expect(actualUrl).to.be.equal('/v2/services?q=label IN ');
//   expect(actualToken).to.be.equal(cfAdminToken);
//   expect(actualPerf).to.be.equal(perf);
//   expect(actualStatistics).to.be.equal(statistics);
// };

const stubSuccessfulCallback = (readPageStub) => {
  readPageStub.callsFake((url, token, perf, statistics,
    callbacks) => {
    // verifyArgs(url, token, perf, statistics);
    callbacks.success();
  });
};

const stubFailingCallback = (readPageStub, error) => {
  readPageStub.callsFake((url, token, perf, statistics,
    callbacks) => {
    // verifyArgs(url, token, perf, statistics);
...
const stubProcessResourceFnCallback = (readPageStub, service, done) => {
  readPageStub.callsFake((url, token, perf, statistics,
    callbacks) => {
    // verifyArgs(url, token, perf, statistics);
...
describe('service-bridge/service-guid-client', () => {
  ...
  context('when no services are provided', () => {
    ...
    beforeEach((done) => {
      stubSuccessfulCallback(paging.readPage);
  ...
  context('when service is provided', () => {
    ...
    context('without guid', () => {
      ...
      context('when call is successful', () => {
        ...
        beforeEach(() => {
          stubProcessResourceFnCallback(paging.readPage, {
            entity: {
              label: 'mongodb'
            },
            metadata: {
      ...
      context('when error occurs', () => {
        ...
        beforeEach((done) => {
          ...
          stubFailingCallback(paging.readPage, error);
