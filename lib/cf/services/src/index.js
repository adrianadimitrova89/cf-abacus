...
const createUsageReporter = (reporter, collectorToken) => {
  return {
    report: (usage, cb) => {
      reporter.reportUsage(usage, collectorToken, cb);
    }
  };
};
...
const createEventFilters = (cfg) => {
  ...
  if (cfg.services)
    filters.push(createServiceFilter(cfg.services));
  return filters;
};
...
const createStatsRoute = (opts) => {
  const route = router();
  if (opts.cfg.oauth.enabled)
    route.use(oauth.authorizer(
      opts.cfg.oauth.jwtKey,
      opts.cfg.oauth.jwtAlgorithm,
      ['abacus.usage.read'])
    );
...
const createWeb = function *(opts) {
  const app = webapp();
  app.use('/v1/stats', createStatsRoute(opts));
