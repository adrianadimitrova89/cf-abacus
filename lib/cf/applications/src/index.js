...
const modifyStartEventUsage = (usage, previousState) => {
  // First/initial start
  if (previousState === states.STOPPED) {
...
const modifyUsage = (usage, event) => {
  ...
  switch (eventState) {
    case states.STARTED:
      ...
      break;
    case states.STAGING_STARTED:
    case states.STAGING_STOPPED:
    case states.BUILDPACK_SET:
...
const reportAppUsage = (cfToken, abacusToken, { failure, success }) => {
  ...
  paging.readPage(url, cfToken, perf, statistics, {
